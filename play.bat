if "%~1"=="" (
	...
	if not "!SelectedFile!"=="" (
		start /min cmd /c "cd %~dp0 && py ecdc_stream.py -d "!SelectedFile!" | ffplay -f s16le -ac 2 -ar 48k -i -"
	) else (
		echo No file selected, exiting.
	)
	endlocal
) else (
	start /min cmd /c "cd %~dp0 && py ecdc_stream.py %2 %3 %4 %5 -d %1 | ffplay -f s16le -ac 2 -ar 48k -i -"
)